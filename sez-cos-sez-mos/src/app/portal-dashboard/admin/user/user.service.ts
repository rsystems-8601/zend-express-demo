...
export class UserService extends RESTAuthClient {
    ...
    @GET("security/users/portal-admin")
    ...
    @ErrorHandler("update-user")
    updateUser(@Body user: UserCreateUpdateRequest): Observable<User> {
        return null;
    }

    @DELETE("security/user/delete/{id}")
    @ErrorHandler("delete-user")
    deleteUser(@Path("id") id: number): Observable<any> {
        return null;
    }

    @GET("security/users/email/uniqueness")
    checkEmailUnique(@Query("userEmail") userEmail: string): Observable<boolean> {
        return null;
    }

    @PUT("security/users/{id}/send-pin")
    ...
    @POST("security/user/search/filter")
     getUsersc3(@Body filter: any): Observable<any> {
         return null;
     }
}
