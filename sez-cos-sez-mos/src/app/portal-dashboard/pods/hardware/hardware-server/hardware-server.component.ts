...
@Component({
  selector: 'app-hardware-server',
  templateUrl: './hardware-server.component.html',
  styleUrls: ['./hardware-server.component.scss']
})
export class HardwareServerComponent implements OnInit {

  columnHeader = ["name", "make", "model", "serialNumber", "cores", "cpuName", "ownerName", "actions"];

  // Dynamic Title and Add button
  titleComponent = "server";
  ...
  // For Paging and filtering
   ...
   reloadData(indexValue?: number, recordPage?: number, fieldName?: any, fieldValue?: any) {
    ...
    this.hardwareService.getHardwaredataTypeAndPodid({"andfilter":{"pod.id":this.podId,"type":"SERVER"},
